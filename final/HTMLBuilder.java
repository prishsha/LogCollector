...
// Dark-themed, card-based, auto-refreshes every 2.5 seconds
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header h1 { font-size: 1.35rem; font-weight:800; color:#fff; letter-spacing:-.01em; }
  header .pulse { width:10px; height:10px; border-radius:50%; background:var(--green);
  ...
  .stat .lbl { font-size:.7rem; color:var(--muted); margin-top:2px; text-transform:uppercase; letter-spacing:.08em; }
  .grid { display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px; }
  .card { background:var(--card); border:1px solid var(--border); border-radius:12px; padding:20px; }
  .card h2 { font-size:.8rem; text-transform:uppercase; letter-spacing:.1em; color:var(--muted);
  ...
  .service-row .name { font-weight:700; font-size:.9rem; color:#fff; }
  .service-row .count { font-family:var(--mono); font-size:.8rem; color:var(--muted); }
  .badge { font-size:.65rem; font-family:var(--mono); padding:3px 10px; border-radius:20px; font-weight:700; }
  .badge.ok    { background:rgba(0,230,118,.15); color:var(--green); border:1px solid rgba(0,230,118,.3); }
  .badge.fail  { background:rgba(255,61,87,.15);  color:var(--red);   border:1px solid rgba(255,61,87,.3); }
  .alert-item { padding:10px 14px; border-radius:8px; margin-bottom:8px; font-size:.82rem;
                font-family:var(--mono); border-left:3px solid; }
  .alert-item.error  { background:rgba(255,61,87,.08);  border-color:var(--red);    color:#ffb3bc; }
  .alert-item.failed { background:rgba(255,215,64,.08); border-color:var(--yellow); color:#ffe082; }
  ...
  thead th { position:sticky; top:0; background:var(--surface); padding:10px 12px;
             text-align:left; color:var(--muted); font-size:.65rem; text-transform:uppercase;
  ...
  .svc  { color:var(--accent); font-weight:700; white-space:nowrap; }
...
""";
    ...
    private static String statsBar(long total, double lps, long uptime, int svcs, int failed) {
        String lpsStr = String.format("%.1f", lps);
        String upStr  = uptime + "s";
        return "<div class='stats-bar'>" +
               stat(String.valueOf(total), "Total Logs") +
               stat(lpsStr, "Logs / sec") +
    ...
    private static String serviceHealthCard(Set<String> all, Set<String> failed,
                                             Map<String, Integer> counts) {
        StringBuilder sb = new StringBuilder();
        sb.append("<div class='card'><h2><span>🖥</span> Service Health</h2>\n");
        if (all.isEmpty()) {
            sb.append("<p class='empty'>No services connected yet.</p>");
        }
        for (String svc : all) {
    ...
    private static String alertsCard(List<LogEvent> logs, Set<String> failed) {
        StringBuilder sb = new StringBuilder();
        sb.append("<div class='card'><h2><span>⚠</span> Recent Alerts</h2>\n");

        int alertCount = 0;
        for (String svc : failed) {
            sb.append("<div class='alert-item failed'>⛔ <b>").append(escHtml(svc)).append("</b> — service has stopped responding</div>\n");
            if (++alertCount >= 5) break;
        }
        for (LogEvent e : logs) {
            if ("ERROR".equals(e.level) && alertCount < 10) {
                sb.append("<div class='alert-item error'>🔴 [").append(e.formattedTime())
        ...
        if (alertCount == 0) {
            sb.append("<p class='empty'>✅ No active alerts</p>");
        }
        sb.append("</div>\n");
        return sb.toString();
    }
    ...
    private static String liveLogsCard(List<LogEvent> logs) {
        ...
        for (LogEvent e : logs) {
            sb.append("<tr>")
              ...
              .append("<td class='msg'>").append(escHtml(e.message)).append("</td>")
              .append("</tr>\n");
        }
